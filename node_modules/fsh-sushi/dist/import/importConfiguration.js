...
function importConfiguration(yaml, file) {
    ...
    const config = {
        ...
        fhirVersion: (_g = normalizeToArray(yaml.fhirVersion)) === null || _g === void 0 ? void 0 : _g.map(v => parseSimpleCode(v, 'fhirVersion', file)),
...
function parseResources(yamlResources, file) {
    ...
    return Object.entries(yamlResources).map(([reference, details]) => {
        ...
        return Object.assign(Object.assign({ reference: { reference } }, details), { profile: normalizeToArray(details.profile), fhirVersion: (_a = normalizeToArray(details.fhirVersion)) === null || _a === void 0 ? void 0 : _a.map(v => parseSimpleCode(v, `resource[${reference}].fhirVersion`, file)) });
    });
}
    ...
    if (!result) {
        ...
        for (const instancePath in errorCollection) {
            ...
            if (additionalPropertiesErrors.length > 0) {
                ...
                FSHLogger_1.logger.warn(`Configuration ${parentProperty}contains unexpected ${propertyWord}: ${additionalPropertyNames}. ${recommendation}`);
            }
            else {
