...
class HasName {
     ...
     */
    setName(fshDefinition) {
        var _a;
        this.name = fshDefinition.name;
        const nameRule = lodash_1.default.findLast(fshDefinition.rules, rule => rule instanceof rules_1.CaretValueRule && rule.caretPath === 'name' && rule.path === '');
        const nameToCheck = nameRule ? nameRule.value : this.name;
        if (!nameRegex.test(nameToCheck)) {
            utils_1.logger.warn(`The name "${nameToCheck}" may not be suitable for machine processing applications such as code generation. Valid names start with an ` +
                "upper-case ASCII letter ('A'..'Z') followed by any combination of upper- or lower-case ASCII letters ('A'..'Z', and " +
                "'a'..'z'), numerals ('0'..'9') and '_', with a length limit of 255 characters.", (_a = nameRule === null || nameRule === void 0 ? void 0 : nameRule.sourceInfo) !== null && _a !== void 0 ? _a : fshDefinition.sourceInfo);
        }
    }
}
...
class HasId {
     ...
     * @param {FshStructure | FshCodeSystem | FshValueSet} fshDefinition - The entity who's id is being set
     */
    validateId(fshDefinition) {
        var _a;
        let idRule;
        if (fshDefinition instanceof fshtypes_1.Instance) {
            idRule = (0, common_1.findIdAssignmentRule)(fshDefinition.rules);
        }
        else {
            idRule = (0, common_1.findIdCaretRule)(fshDefinition.rules);
        }
        const idToCheck = idRule ? idRule.value : this.id;
        let validId = primitiveTypes_1.idRegex.test(idToCheck);
        if (!validId && !idRule && nameRegex.test(this.id) && !(fshDefinition instanceof fshtypes_1.Instance)) {
            ...
            if (primitiveTypes_1.idRegex.test(sanitizedId)) {
                ...
                utils_1.logger.warn(`The string "${this.id}" represents a valid FHIR name but not a valid FHIR id. FHIR ids cannot contain "_" and can be at most 64 characters. The id will be exported as "${sanitizedId}". Avoid this warning by specifying a valid id directly using the "Id" keyword.`, fshDefinition.sourceInfo);
                this.id = sanitizedId;
                validId = true;
            }
        }
        if (!validId) {
            ...
            utils_1.logger.error(message, (_a = idRule === null || idRule === void 0 ? void 0 : idRule.sourceInfo) !== null && _a !== void 0 ? _a : fshDefinition.sourceInfo);
        }
    }
}
exports.HasId = HasId;
